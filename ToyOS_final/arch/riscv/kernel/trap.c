...
void do_page_fault(struct pt_regs *regs) {
    ...
    //fs_tset();
    if(regs->scause==12){
 	    create_mapping(current->pgd,vma->vm_start,(uint64)(uapp_start)-PA2VA_OFFSET,vma->vm_end-vma->vm_start,vma->vm_flags+8);
         //create_mapping(current->pgd,)
    }else if(regs->scause==13||regs->scause==15){
        create_mapping(current->pgd,vma->vm_start,current->thread_info->user_sp-PGSIZE,vma->vm_end-vma->vm_start,vma->vm_flags+8);
    }
    /*
...
uint64 do_fork(struct pt_regs *regs) {
    ...
    for(j=0;j<512;j++)
    {
        ...
        child->pgd[j]=current->pgd[j];
    }
    //create_mapping(child->pgd,USER_END-PGSIZE,child -> thread_info->user_sp,PGSIZE,7);
    ...
    else if(temp == 0)
    {
        ...
        if(temp == 8)//Environment call from U-mode
        {
            ...
            }else if(regs->a7==SYS_WRITE)//64
            {
                ...
                regs->sepc+=4;
            }else if(regs->a7==SYS_WAIT){//260
                regs->a0=sys_wait(regs);
                regs->sepc+=4;
            }else if(regs->a7==SYS_EXIT){//93
                sys_exit(regs);
                regs->sepc+=4;
            }else if(regs->a7==SYS_EXECVE){//221
                regs->a0=sys_exec(regs->a0);
                regs->sepc+=4;
            }
        }
